...
class Scenario:
     def __init__(self, nscc, scenario_dir, scenario_name, debug=False):
        ...
        self.valid_scenarios = sorted([ y[1] for y in [str(x[0]).split('/') for x in os.walk('.')][1:]])
     ...
     def execute(self):
         ...
         if not self.scenario_name in self.valid_scenarios:
            print("\n\tInvalid Scenario Specified")
            exit(0)

         self.scenario = self.get_json_file(self.path + self.scenario_name + ".json")
...
def main(argv):
     ...
     parser.add_argument("-l", "--list", action="store_true", dest='list', help="List valid scenario name(s)")
     parser.add_argument("-s", "--scenario", action="store", dest='scenario', default="none", nargs='?', help="Scenario Name")
     ...
     if args.list:
       ...
       print("\t\t--------------------------")
       for nm in scenario.valid_scenarios:
           print("\t\t{0}".format(nm))
       print("")
       exit(0)
